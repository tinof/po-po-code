...
const PROMPTS_DIR_NAME = 'po-po-code';
...
function loadConfigFromPath(configPath: string): PluginConfig | null {
  try {
    ...
    if (!result.success) {
      console.warn(`[po-po-code] Invalid config at ${configPath}:`);
    ...
    if (
      ...
      console.warn(
        `[po-po-code] Error reading config from ${configPath}:`,
        error.message,
      );
    }
    return null;
 ...
 *
 * @param basePath - Base path without extension (e.g., /path/to/po-po-code)
 ...
 * 1. User config: $OPENCODE_CONFIG_DIR/po-po-code.jsonc or .json,
 *    or ~/.config/opencode/po-po-code.jsonc or .json (or $XDG_CONFIG_HOME)
 * 2. Project config: <directory>/.opencode/po-po-code.jsonc or .json
 *
...
export function loadPluginConfig(directory: string): PluginConfig {
  ...
  const projectConfigBasePath = path.join(directory, '.opencode', 'po-po-code');
  ...
  if (config.preset) {
    ...
    if (preset) {
      ...
      const availablePresets = config.presets
        ...
        : 'none';
      console.warn(
        `[po-po-code] Preset "${config.preset}" not found (from ${presetSource}). Available presets: ${availablePresets}`,
      );
    }
  }
  ...
  const readFirstPrompt = (
    ...
    for (const dir of promptSearchDirs) {
      ...
      try {
        ...
        console.warn(
          `[po-po-code] ${errorPrefix} ${promptPath}:`,
